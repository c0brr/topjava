...
public class ExceptionInfoHandler {
    ...
    public ErrorInfo conflict(HttpServletRequest req, DataIntegrityViolationException e) {
        ...
        return logAndGetErrorInfo(req, e, true, DATA_ERROR, error);
    }
    ...
    public ErrorInfo bindingError(HttpServletRequest req, BindException e) {
        String[] errorsArray = e.getBindingResult().getFieldErrors().stream()
                .map(fe -> "[" + fe.getField() + "] " + fe.getDefaultMessage())
                .toArray(String[]::new);
        return logAndGetErrorInfo(req, e, false, VALIDATION_ERROR, errorsArray);
    }

    //    https://stackoverflow.com/questions/538870/should-private-helper-methods-be-static-if-they-can-be-static
    private static ErrorInfo logAndGetErrorInfo(HttpServletRequest req, Exception e, boolean logException,
                                                ErrorType errorType, String... errors) {
        ...
        return new ErrorInfo(req.getRequestURL(), errorType, errors.length == 0 ?
                new String[]{rootCause.toString()} : errors);
    }
}